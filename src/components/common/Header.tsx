...
const formatNotificationTime = (item: NotificationItem): string => {
  if (item.createdAt) {
    ...
    if (!isNaN(d.getTime())) {
      ...
      const hoursStr = String(d.getHours()).padStart(2, '0') + 'h' + String(d.getMinutes()).padStart(2, '0');

      if (diffMinutes < 1) {
        return "À l'instant";
      }
      if (diffMinutes < 60) {
        return `Il y a ${diffMinutes} min (${hoursStr})`;
      }
      
      const isToday = d.toDateString() === now.toDateString();
      if (isToday) {
        return `Aujourd'hui à ${hoursStr}`;
      }

      const yesterday = new Date(now);
      yesterday.setDate(now.getDate() - 1);
      if (d.toDateString() === yesterday.toDateString()) {
        return `Hier à ${hoursStr}`;
      }

      const day = String(d.getDate()).padStart(2, '0');
      const month = String(d.getMonth() + 1).padStart(2, '0');
      return `${day}/${month} à ${hoursStr}`;
    }
  }
  return item.time || "Aujourd'hui";
};
...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  const handleRetrySync = async () => {
    ...
    try {
      await storage.retryCloudConnection();
    } finally {
      setTimeout(() => setIsRetryingSync(false), 500);
    }
  };

  // Sync notifications and syncStatus with storage, and tick time every minute
  useEffect(() => {
    const update = () => {
    ...
    <header className="h-16 bg-white border-b border-slate-200/80 px-3 sm:px-6 lg:px-8 flex items-center justify-between sticky top-0 z-30 shadow-xs">
        ...
        <div className="flex items-center">
          ...
          {(syncStatus === 'offline' || syncStatus === 'error') && (
            <button
              ...
              title="Mode local temporaire. Cliquez pour retester et reconnecter immédiatement au Cloud."
              className="flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 active:scale-95 transition-all border border-slate-300 px-2.5 py-1 rounded-full text-xs font-medium shadow-xs cursor-pointer group"
            >
              <CloudOff className="w-3.5 h-3.5 text-slate-500 group-hover:text-slate-700" />
              <span className="hidden sm:inline">Mode local</span>
              <RefreshCw className={`w-3 h-3 text-slate-400 group-hover:text-slate-600 ml-0.5 ${isRetryingSync ? 'animate-spin text-[#EA580C]' : ''}`} />
            </button>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-1 text-[10px] text-slate-400">
                          <span title={n.createdAt ? new Date(n.createdAt).toLocaleString('fr-FR') : n.time}>
        ...
        <div className="relative" ref={userMenuRef}>
          ...
          >
            <div 
              className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs text-white shadow-xs border border-white/20 flex-shrink-0 ${
                currentUser.role === 'ADMIN' ? 'bg-[#EA580C]' : 'bg-slate-800'
              }`}
            >
          ...
          {userMenuOpen && (
            ...
            >
              <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-3">
                <div 
                  className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm text-white shadow-xs border border-white/20 flex-shrink-0 ${
                    currentUser.role === 'ADMIN' ? 'bg-[#EA580C]' : 'bg-slate-800'
                  }`}
                >
                  {getUserInitials(currentUser.name)}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-orange-100 text-[#EA580C]">
